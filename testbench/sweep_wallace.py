...
Supports Dadda, Array, and Wallace Tree architectures.
"""
import argparse
import csv
import os
...
def build_gold(genmul, in1, in2, workdir, timeout, signed_mode=False, arch="wallace", verbose=False):
...
def check_one(genmul, abc_bin, gold_aig, topmod, in1, in2, column, cout_mask, sum_mask,
              method, timeout, keep_netlists, keep_dir, signed_mode=False, arch="wallace", verbose=False):
    t0 = time.time()
    ppg = 2 if signed_mode else 1
...
def main():
    ...
    ap.add_argument("--arch", choices=["array", "dadda", "wallace"], default="wallace")
    ap.add_argument("--column", type=int, default=None, help="Specific column to approximate.")
    ...
    if shutil.which(abc_bin) is None:
        if shutil.which("abc") is not None:
            ...
            sys.exit(f"ABC not found. Please pass --abc.")
    ...
    columns_to_test = [args.column] if args.column is not None else list(range(1, max_column + 1))
    ...
    rng = random.Random(args.seed if args.seed != 0 else time.time())
    
    if args.masks == "all":
        combos = [(col, c, s) for col in columns_to_test for c in range(256) for s in range(256)]
    else:
        if args.column is None and args.sample_size <= len(columns_to_test):
            sampled_cols = rng.sample(columns_to_test, args.sample_size)
            combos = [(col, rng.randint(0, 255), rng.randint(0, 255)) for col in sampled_cols]
        else:
            pool = [(col, c, s) for col in columns_to_test for c in range(256) for s in range(256)]
    ...
    if excluded:
        combos = [(col, c, s) for (col, c, s) in combos if (c, s) not in excluded]

    done = load_done(csv_path) if args.resume else set()
    todo = [(col, c, s) for (col, c, s) in combos if (col, c, s) not in done]
        ...
        with ProcessPoolExecutor(max_workers=args.jobs) as pool:
            ...
            for fut in as_completed(futures):
                ...
                if result["status"] != "PASS":
                    ...
                    print(f"[{n_done}/{len(todo)}] PASS col={result['column']} ({rate:.2f} combos/s, ~{remaining/60:.1f} min remaining)")
