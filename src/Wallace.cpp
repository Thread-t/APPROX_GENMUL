...
static vector<int> WallaceCore(map<int, int> Ins, int nIn1, int nIn2, string &file,
                              const string &typeName, int truncateBits, int approxColumn = -1,
    ...
    while (!stopFlag)
    {
        for (auto &i : LevelizedPartials)
        {
            while (i.second.size() >= 3)
            {
                ...
                comp = new FullAdder({i.second[0], i.second[1], i.second[2]}, useApproxFA, false, useApproxFA && debugMode);
                comp->SetOutputs();
                         ...
                         int approxColumn, int approxMethod, bool debugMode)
{
    ...
    return WallaceCore(Ins, nIn1, nIn2, file, "AWT", truncateBits, approxColumn, debugMode);
}
...
{
    return WallaceCore(Ins, nIn1, nIn2, file, "WT", 0, -1, false);
}
